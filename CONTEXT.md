...
- The Surface is flat and typography-first, with one optional accent: a person may let the current Workspace's own colour fill the pill, the ring and the selected row, and monochrome is the default. Opaque neutral surfaces separate only where a relationship requires a hairline; hierarchy otherwise comes from type and spacing. Hue is reserved for the three things that identify rather than style — the provider mark, a Workspace's own swatch, and the one destructive action. Status is carried by the shape of a glyph, never by its colour. Light and dark are both first-class.
