...
- The Surface is one shell with one opaque pane inset on it, and typography-first inside the pane, with one optional accent: a person may let the current Workspace's own colour fill the pill, the ring and the selected row, and monochrome is the default. The pane's border is the only persistent edge; inside it, surfaces separate only where a relationship requires a hairline, and hierarchy otherwise comes from type and spacing. Blur is native under the shell or on popovers, never over content. Hue is reserved for the three things that identify rather than style — the provider mark, a Workspace's own swatch, and the one destructive action. Status is carried by the shape of a glyph, never by its colour. Light and dark are both first-class.
- Real document content is paper: when Deskto shows the file it produced rather than an agent talking about it, that surface leaves the chrome system and becomes an opaque lifted sheet in both themes.
