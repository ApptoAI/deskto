...
- The Surface is monochrome glass over a wallpaper, with one optional accent: a person may let the current Workspace's own colour fill the pill, the ring and the selected row, and monochrome is the default. Every surface, edge, and text colour is the canvas or its opposite at some opacity; hue is reserved for the three things that identify rather than style — the provider mark, a Workspace's own swatch, and the one destructive action. Status is carried by the shape of a glyph, never by its colour. Light and dark are the same rules read from opposite ends, and both are first-class.
- Real document content is paper: when Deskto shows the file it produced rather than an agent talking about it, that surface leaves the glass system and becomes an opaque lifted sheet in both themes.
