...
An activity may carry an `ActivityPayload` classifying it into a provider-neutral kind: `tool` (command, search, web, mcp, other), `file-change` with a path list, `plan` with steps, or `subagent`. `activity.updated` exists for the kinds that change while they run, a plan above all. `parentId` names the provider id of the activity this one runs inside, which is how a subagent's tool calls nest under the call that spawned it. Payloads are bounded summaries: no transcripts, no raw tool output (see `docs/adr/0007-typed-activities-and-thread-deltas.md`).
