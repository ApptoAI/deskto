...
`HarnessRunInput` carries the thread, turn, project path, prompt, execution profile, a `customization` value (today: `skillRoots`, absolute paths to directories of SKILL.md skill folders the adapter should make available to the session), and an optional `providerSessionId`, the opaque token that lets a provider resume an earlier session. Adapters translate the customization into native mechanisms and silently skip what the installed harness version cannot honor.
