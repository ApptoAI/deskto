...
A `HarnessSession` exposes `events` (an `AsyncIterable<HarnessEvent>`), `cancel()`, and `respondToApproval(approvalId, decision)`. The event union has ten members: `session.started`, `progress.updated`, `message.delta`, `usage.updated`, `activity.started`, `activity.updated`, `activity.completed`, `approval.requested`, `turn.completed`, and `turn.failed`. `progress.updated` is a bounded liveness signal and never contains reasoning text or raw tool arguments. A session emits at most one unresolved approval request at a time, and `usage.updated` reports how full the provider's context window is (`ContextUsage`).

An activity may carry an `ActivityPayload` classifying it into a provider-neutral kind: `tool` (command, search, web, mcp, other), `file-change` with a path list, `plan` with steps, or `subagent`. `activity.updated` exists for the kinds that change while they run, a plan above all. `parentId` names the provider id of the activity this one runs inside, which is how a subagent's tool calls nest under the call that spawned it. Payloads are bounded summaries: no transcripts, no raw tool output (see `docs/adr/0008-typed-activities-and-thread-deltas.md`).
