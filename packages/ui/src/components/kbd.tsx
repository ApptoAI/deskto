...
function Kbd({ className, ...props }: ComponentProps<"kbd">) {
  return (
    <kbd
      data-slot="kbd"
      className={cn(
        "inline-flex h-5 shrink-0 items-center justify-center rounded-key bg-foreground/6 px-1.5 font-mono text-tiny leading-none text-muted-foreground tabular-nums select-none",
        className
      )}
      {...props}
    />
