...
Requests are one discriminated union, `runtimeRequestSchema`, keyed on `method`. There are 13 methods across five groups: `harness.*` (list, setEnabled, refresh), `preferences.get`, `workspace.*` (list, add), `thread.*` (list, create, configure, get), `turn.*` (start, cancel), and `approval.resolve`. The Runtime validates incoming requests with this schema at the IPC boundary.
...
Events are a second discriminated union with two members: `thread.changed` (with a `threadId`) and `harness.changed`. Events carry no payloads beyond the id. They tell an open view to refetch; they are not a data stream.
