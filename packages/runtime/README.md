...
- The request router dispatches protocol methods such as `project.add`, `workspace.create`, `pack.install`, `pack.link`, `settings.update`, `turn.start`, and `approval.resolve`. It wraps results in the protocol's `{ ok, data | error }` envelope. Managed Packs live under the host's `packsPath`; linked Packs stay at their original paths.
