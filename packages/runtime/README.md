...
- The turn coordinator runs a turn: it persists the user message first, starts a harness session, then consumes the session's event stream and turns it into stored messages, activities, and approvals, emitting `thread.changed` so open views refetch. On a Thread's first Turn it also starts an independent title generation call; failure leaves the `New task` seed in place and never blocks the Turn.
